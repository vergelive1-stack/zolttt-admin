...
exports.index = async (req, res) => {
  try {
    ...
    const [total, song] = await Promise.all([
      Song.countDocuments({ isDelete: false }),
      Song.find({ isDelete: false })
        .skip((start - 1) * limit)
        .limit(limit)
        .sort({ createdAt: -1 })
        .lean(),
    ...
    return res.status(200).json({ status: true, message: "Success!!", total, song });
  } catch (error) {
    console.log(error);
